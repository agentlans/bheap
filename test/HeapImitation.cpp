...
void do_random_action(HeapImitation& h, binary_heap* bh) {
	...
	case 0:
		...
		compare<int>(handle, handle_c, "inserted handle");
		break;
	case 1:
		// Get
		x = h.get(handle);
	...
	case 3:
		...
		std::cout << "Success? " << success << ". Error? " << c_error << std::endl;
		assert(success xor c_error);
		break;
	case 4:
		// Top
		...
		compare<int>(handle, handle_c, "top handle");
	...
	case 5:
		...
		compare<int>(handle, handle_c, "popped handle");
		compare<double>(x, x_c, "popped value");
		break;
	}
	//binary_heap_print(bh);
	compare<uint>(h.size(), binary_heap_size(bh), "heap size");
}
